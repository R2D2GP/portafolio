...
| 2026-07-16 | 29 | AIStack.tsx, DOCUMENTACION.md | **Fade-in en detail card de AI Stack**: Se envolvió la línea decorativa SVG + el detail card (con su AnimatePresence) en un `motion.div` con `initial={{ opacity: 0, y: 15 }}` y `animate` controlado por `isInView`, con `delay: 0.3s` para escalonar la entrada después del heading (0s) y los botones (0.15s). | La animación de cambio entre categorías (slide direction) no se ve afectada porque el AnimatePresence interno sigue funcionando independientemente del wrapper exterior. Build: 0 errores. Tests: 62 pasan, 10 skip, 0 fallos. |
