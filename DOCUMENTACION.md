...
|---|-------------|--------|----------|
| 1 | **Animación carrusel móvil AI Stack desincronizada** | 🟡 Pospuesto | Después de múltiples interacciones rápidas con los botones `< >` en mobile, la dirección de entrada/salida de la animación se desincroniza. La prop `custom` de Framer Motion captura `slideDirection` correctamente, pero en ciertos escenarios las animaciones pueden entrar/salir del mismo lado. Requiere repensar la lógica: posible solución es usar `useRef` para capturar el estado del slide inmediatamente antes del unmount, o usar `layout ID` de Framer Motion para mantener consistencia. |
